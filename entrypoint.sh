...
if [ "$CLEAN_UP" = "true" ]; then
  ...
  a="ssh_execute_remote $SSH_HOST $SSH_PORT $SSH_USER $SSH_HOST_IP delete $CPANEL_URL $CPANEL_USER '$CPANEL_KEY' $DATABASE_HOST $DATABASE_NAME $DATABASE_USER ${DATABASE_PASSWORD@Q} $PATH_PUBLIC $DEPLOY_DOMAIN $DEPLOY_SUBDOMAIN"
  echo $a
